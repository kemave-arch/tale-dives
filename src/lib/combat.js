...
export function statsForTier(tier, level) {
  const t = TIER_BASELINES[tier] ?? TIER_BASELINES.standard
  return {
...
// entering COMBAT needs a full stat block. lib/codex.js's keyword-link pass
...
export function ensureAdversary(bestiary, id, name, tier, level) {
  const dict = bestiary ?? {}
  ...
  const entry = { name: existing?.name ?? name, threatTier: tier, hpMax, dmgBase, autoLogged: true }
...
export function computePlayerAttack(player) {
...
export function isDisengaging(actionText) {
  return DISENGAGE_RE.test(actionText)
}

// §3.1 Combat Result context-slice line, only present on a Tactical attack turn.
export function describeCombatResult({ enemyName, damage, enemyHp, enemyHpMax, defeated, playerDamageTaken, exhausted }) {
