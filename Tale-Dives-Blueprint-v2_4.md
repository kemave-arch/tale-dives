...
3. **Equipped Quick-Slots**: Selecting 3 active skills for instant 1-tap cast buttons on the UI tray — pre-populated from `suggested_quick_slots` when the class came from grounding.
...
1. **Action Input**: Player types an action or selects a suggested choice pill / quick-slot skill.
...
A floating action control that keeps lower-frequency systems reachable from the parchment view without permanent chrome eating screen space. **Quick-Slots are deliberately excluded** — see the Quick-Slot Tray in §6.4C — since a menu the player has to open first is the wrong home for the action they take most often; the radial fan is for navigation and management actions instead.
...
* **Thumb-reachable action tray**: quick-slots and the action input anchor to the bottom of the viewport, not the top — the single highest-impact mobile layout decision for a text-input-heavy app.
...
* **Large touch targets** for choice pills and quick-slots (44×44px minimum), with generous spacing — dense desktop-style click targets are the most common mobile-port complaint.
